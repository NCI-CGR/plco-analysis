...
void process_data(const std::vector<std::string> &input_filenames,
		  ...
		  const std::vector<std::map<std::pair<unsigned, unsigned>, std::map<std::string, std::pair<unsigned, unsigned> > > > &combinatorial_file_counts) {
  ...
  try {
	...
	if (input_lines.at(i).empty()) {
	  ...
	  while (inputs.at(i)->getline(input_lines.at(i))) {
	    ...
	    if (!(strm1 >> input_pvalues.at(i) >> catcher >> catcher >> input_n.at(i).first >> input_n.at(i).second))
	      throw std::domain_error("insufficient entries for file \"" + input_filenames.at(i) + "\" line \"" + input_lines.at(i) + "\"");
	    break;
	  }
	...
	for (unsigned i = 0; i < input_n.size(); ++i) {
	  ...
	  std::map<std::pair<unsigned, unsigned>, std::map<std::string, std::pair<unsigned, unsigned> > >::const_iterator comparison_finder = combinatorial_file_counts.at(comparison_number).find(input_n.at(i));
	  if (comparison_finder == combinatorial_file_counts.at(comparison_number).end()) {
	    std::ostringstream o_exception;
	    o_exception << "combinatorial sample size lookup failed: comparison " << comparison_number << "; expected " << input_n.at(i).first << "/" << input_n.at(i).second << std::endl;
	    o_exception << "available:";
	    for (std::map<std::pair<unsigned, unsigned>, std::map<std::string, std::pair<unsigned, unsigned> > >::const_iterator except_iter = combinatorial_file_counts.at(comparison_number).begin(); except_iter != combinatorial_file_counts.at(comparison_number).end(); ++except_iter) {
	      o_exception << ' ' << except_iter->first.first << '/' << except_iter->first.second;
	    }
...
void compute_combinatorial_uniques(const std::vector<std::string> &model_matrix_filenames,
				   std::vector<std::map<std::pair<unsigned, unsigned>, std::map<std::string, std::pair<unsigned, unsigned> > > > &res) {
  ...
  for (unsigned comp = 1; comp < subjects_by_file.size(); ++comp) {
    ...
    for (unsigned i = 1; i < static_cast<unsigned>(pow(2, subjects_by_file.at(comp).size()) + 0.5); ++i) {
      ...
      unsigned map_count = 0, running_total_cases = 0, running_total_controls = 0;
	...
	if (i & (1 << map_count)) {
      ...
      if (res.at(comp).find(std::make_pair(running_total_cases, running_total_controls)) != res.at(comp).end()) {
	std::ostringstream o_exception;
	o_exception << "hackjob sample size resolution failed due to coincidental collision of total sample sizes: size is "
		    << running_total_cases << " cases and " << running_total_controls << " controls" << std::endl;
	o_exception << "previously stored values are:";
	for (std::map<std::pair<unsigned, unsigned>, std::map<std::string, std::pair<unsigned, unsigned> > >::const_iterator except_iter = res.at(comp).begin();
	...
	throw std::runtime_error(o_exception.str());
      ...
      res.at(comp)[std::make_pair(running_total_cases, running_total_controls)] = alternate_lookup;
    }
  }
}
