    ...
    if (!qq.only) {
        ...
        if (!is.na(filename.prev.hits)) {
            ...
            for (i in 1:nrow(prev.hits)) {
                ...
                if (length(raw.data[,pos.col.header][raw.data[,rsid.col.header] == snp]) >= 1) {
                    locus.pos.center <- raw.data[,pos.col.header][raw.data[,rsid.col.header] == snp][1]
                    ...
                    snp.label.colors <- c(snp.label.colors, prev.loci.color)
                                        ...
                                        #for each set of previously unknown loci, overwrite the color as novel.loci.color
